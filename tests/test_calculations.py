...
class TestDatadogMetrics:
    def test_metrics_from_overview(self):
        """Metrics Overview is preferred when available (per Notion guide)."""
        ext = DatadogExtraction(
    ...
    def test_metrics_discrepancy_warning(self):
        """Big gap between Overview and calculated triggers a DISCREPANCY warning."""
        ext = DatadogExtraction(
            total_metrics_from_overview=10_000_000,
            infra_hosts=10,  # calculated: 10 × 750 = 7,500
        )
        result = dd_calculate(ext)
        assert result.metrics_num_series == 10_000_000  # uses Overview
...
class TestTrigoReference:
    ...
    def test_trigo_metrics_with_overview(self):
        """When Metrics Overview is provided, it takes priority over calculated."""
        ext = DatadogExtraction(
            infra_hosts=249,
            apm_hosts=77,
            network_hosts=25,
            container_hours=1_080_000,
            custom_metrics=167_000,
            total_metrics_from_overview=4_090_000,
        )
        result = dd_calculate(ext)
        # Uses Metrics Overview value per Notion guide
        assert result.metrics_num_series == 4_090_000

    def test_trigo_metrics_without_overview(self):
        """Without Metrics Overview, falls back to calculated value."""
        ext = DatadogExtraction(
            infra_hosts=249,
            apm_hosts=77,
            network_hosts=25,
        ...
        result = dd_calculate(ext)
        # Calculated: (249+77+25+1650+0)×750 + 167000 ≈ 1.67M
        assert result.metrics_num_series > 1_500_000
        assert result.metrics_num_series < 1_800_000
    ...
    def test_metrics_overview_discrepancy_warning(self):
        """When overview differs >2x from calculated, emit DISCREPANCY warning."""
        ext = DatadogExtraction(
            infra_hosts=100,
        ...
        result = dd_calculate(ext)
        # Uses Overview (5M), calculated is 125K — big gap triggers warning
        assert result.metrics_num_series == 5_000_000
        assert any("DISCREPANCY" in w for w in result.warnings)
